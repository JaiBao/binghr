    ...
    >
      <template v-slot:top>
        ...
        <q-select
          ...
          class="q-mx-xs"
        />
        <div class="text-h6 q-mx-md">月打卡紀錄</div>
...
<script setup>
import { ref, reactive, watch, computed, onMounted } from 'vue' // computed
...
watch(() => select.value.month, (newValue, oldValue) => {
...
if (hours.shouldChangeMonth) {
  ...
  select.value.year = hours.year
  // getAllTime()
  hours.shouldChangeMonth = false
}
onMounted(() => {
  setTimeout(() => {
    getAllTime()
